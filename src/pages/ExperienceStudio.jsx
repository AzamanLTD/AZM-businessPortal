...
export default function ExperienceStudio() {
  ...
  useEffect(() => {
    if (experienceQuery.data?.blueprint) setDraft(experienceQuery.data.blueprint);
  }, [experienceQuery.data?.blueprint]);
  ...
  const previewQuery = useQuery({
    ...
    queryFn: async () => {
      if (category === 'FOOD_BEVERAGE' || category === 'RESTAURANT' || category === 'RETAIL' || category === 'SERVICE' || category === 'OTHER') {
        ...
        return { products: response?.products || response?.data?.products || [], rooms: [], trips: [] };
      }
      if (category === 'HOSPITALITY' || category === 'HOTEL') {
        const response = await hotelOpsApi.getRooms();
        return { products: [], rooms: response?.rooms || response?.data?.rooms || [], trips: [] };
      }
      if (category === 'LOGISTICS' || category === 'TRANSIT') {
        const response = await transit.list();
        return { products: [], rooms: [], trips: response?.trips || response?.data?.trips || [] };
      }
      return { products: [], rooms: [], trips: [] };
    },
  });

  const saveMutation = useMutation({
    mutationFn: (blueprint) => storefrontApi.saveExperience(blueprint),
    onSuccess: (saved) => {
      setDraft(saved);
      setMessage('Saved as a storefront draft. Publish your storefront from Storefront Editor to make this experience live.');
      queryClient.setQueryData(['experience-studio', 'experience'], (current) => current ? { ...current, blueprint: saved } : current);
    },
  });

  const navigationModes = (experienceQuery.data?.navigationModes || []).filter((mode) => policy.navigationModes.includes(mode));
  const detailPresentations = (experienceQuery.data?.detailPresentations || []).filter((presentation) => policy.detailPresentations.includes(presentation));
  ...
  const preview = previewQuery.data || { products: [], rooms: [], trips: [] };
  ...
  return <div className="min-h-full p-6 lg:p-8"><div className="mx-auto max-w-6xl space-y-6">
    <header className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between"><div><p className="text-[11px] font-semibold uppercase tracking-[0.22em]" style={{ color: 'var(--accent)' }}>Experience Studio</p><h1 className="mt-2 text-2xl font-bold tracking-tight" style={{ color: 'var(--text)' }}>Design how customers move through your business</h1><p className="mt-2 max-w-2xl text-sm leading-6" style={{ color: 'var(--text-2)' }}>This is not a page builder. AZM owns the interaction grammar and accessibility; you control the character, context and transaction feel.</p></div><button type="button" onClick={save} disabled={saveMutation.isPending} className="rounded-xl px-5 py-3 text-sm font-semibold text-white disabled:opacity-60" style={{ background: 'var(--accent)' }}>{saveMutation.isPending ? 'Saving…' : 'Save experience'}</button></header>
    {(message || loadError) && <div className="rounded-xl border px-4 py-3 text-sm" style={{ borderColor: loadError ? 'var(--danger)' : 'var(--line)', color: loadError ? 'var(--danger)' : 'var(--text-2)', background: 'var(--surface)' }}>{loadError || message}</div>}
    <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
      <div className="space-y-6">
        <Section title="Journey preset" description="The preset determines the interaction language available to customers. Your business type constrains what can be selected."><div className="grid gap-3 sm:grid-cols-2">{policy.presets.map((preset) => { const meta = PRESET_META[preset]; const active = draft.preset === preset; return <button key={preset} type="button" onClick={() => setDraft((current) => ({ ...current, preset }))} className="text-left rounded-xl border p-4 transition" style={{ borderColor: active ? 'var(--accent)' : 'var(--line)', background: active ? 'color-mix(in srgb, var(--accent) 8%, var(--surface))' : 'var(--surface)' }}><div className="flex items-center justify-between gap-3"><span className="text-sm font-semibold" style={{ color: 'var(--text)' }}>{meta?.title || preset}</span>{active && <span className="text-[10px] font-bold uppercase tracking-wider" style={{ color: 'var(--accent)' }}>Active</span>}</div><p className="mt-2 text-xs leading-5" style={{ color: 'var(--text-3)' }}>{meta?.description || 'Category-native journey preset.'}</p></button>; })}</div></Section>
        ...
        <Section title="Motion personality" description="Motion changes pacing, not meaning. Reduced-motion users always receive a safe non-motion equivalent."><div className="grid gap-3 sm:grid-cols-3">{(experienceQuery.data?.motionTempos || []).map((tempo) => <button key={tempo} type="button" onClick={() => patch('motion.tempo', tempo)} className="rounded-xl border px-4 py-3 text-sm font-semibold" style={{ borderColor: draft.motion.tempo === tempo ? 'var(--accent)' : 'var(--line)', color: 'var(--text)' }}>{tempo[0] + tempo.slice(1).toLowerCase()}</button>)}</div></Section>
