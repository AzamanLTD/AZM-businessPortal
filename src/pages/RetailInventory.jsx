  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-xl font-bold text-[var(--az-text)]">Retail Inventory</h1>
      ...
      <div className="grid grid-cols-4 gap-3">
        ...
        <Card className="p-4">
          <div className="flex items-center gap-2 mb-1">
            <ClipboardCheck className="w-4 h-4 text-[var(--az-accent)]" />
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 text-sm font-medium border-b-2 transition-colors ${
                activeTab === tab.key
                  ? 'border-emerald-500 text-[var(--az-success)]'
                  : 'border-transparent text-az-text-secondary hover:text-[var(--az-text)]'
              }`}
            >
...
function OverviewTab({ lowStockItems, loading }) {
  ...
  return (
    <div className="space-y-6">
      <Card className="p-5">
        <h2 className="text-sm font-semibold text-[var(--az-warning)] uppercase tracking-wide mb-3 flex items-center gap-2">
            ...
            {lowStockItems.map(item => (
              <div key={item.id} className="flex items-center justify-between bg-az-card rounded-lg p-3 border border-az-border/30">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-[var(--az-text)]">{item.name}</p>
                    <p className="text-xs text-az-text-muted">SKU: {item.sku || '—'}</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-sm font-bold text-[var(--az-warning)]">{item.stockQty || 0} units</p>
      ...
      <Card className="p-5">
        <h2 className="text-sm font-semibold text-[var(--az-info)] uppercase tracking-wide mb-3 flex items-center gap-2">
              ...
              <tbody>
                {products.slice(0, 30).map(product => (
                  <tr key={product.id} className="border-b border-az-border/30 hover:bg-az-card/50">
                    <td className="py-2 pr-3 text-[var(--az-text)]">{product.name}</td>
                    {editingId === product.id ? (
                      ...
                      <>
                        ...
                        <td className="py-2 px-3">
                          <span className={`text-xs font-medium ${product.stockQty !== null && (product.stockQty || 0) <= (product.lowStockThreshold || 5) ? 'text-[var(--az-warning)]' : 'text-[var(--az-text)]'}`}>
                            {product.stockQty ?? '—'}
                          </span>
                        </td>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {suppliers.map(s => (
            <Card key={s.id} className="p-4">
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-sm font-medium text-[var(--az-text)]">{s.name}</p>
              ...
              <div className="flex gap-2 mt-3">
                ...
                <Button size="sm" variant="outline" onClick={() => { if (confirm(`Delete supplier "${s.name}"?`)) deleteMut.mutate(s.id); }}>
                  <Trash2 className="w-3 h-3 text-[var(--az-danger)]" />
                </Button>
              </div>
            </Card>
          ))}
          ...
          {purchaseOrders.map(po => (
            <Card key={po.id} className="p-4 cursor-pointer hover:border-az-border/60 transition-colors" onClick={() => setSelectedPO(po)}>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-lg bg-[var(--az-success)]/15 flex items-center justify-center flex-shrink-0">
                    <FileText className="w-5 h-5 text-[var(--az-success)]" />
                  </div>
                  <div>
                    <p className="text-sm font-medium text-[var(--az-text)]">{po.poNumber}</p>
                    <p className="text-xs text-az-text-muted">{po.supplier?.name || 'Unknown'} • {po.items?.length || 0} item{po.items?.length !== 1 ? 's' : ''}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <div className="text-right">
                    <p className="text-sm font-bold text-[var(--az-text)]">{fmtUSDC(po.totalCost)}</p>
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} title="Create Purchase Order" size="lg">
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="flex justify-end mt-2"><span className="text-sm font-bold text-[var(--az-text)]">Total: {fmtUSDC(poTotal)}</span></div>
          </div>
          <div>
            <label className="text-xs text-az-text-secondary mb-1 block">Notes</label>
      ...
      {selectedPO && (
        <Modal open={!!selectedPO} onClose={() => setSelectedPO(null)} title={selectedPO.poNumber} size="lg">
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-az-text-secondary">Supplier: <span className="text-[var(--az-text)]">{selectedPO.supplier?.name}</span></p>
                ...
                <tbody>
                  {selectedPO.items?.map(item => (
                    <tr key={item.id} className="border-b border-az-border/30">
                      <td className="py-1.5 pr-2 text-[var(--az-text)]">{item.productName}</td>
                      <td className="py-1.5 px-2 text-az-text-secondary font-mono text-xs">{item.sku || '—'}</td>
                      <td className="py-1.5 px-2 text-right text-[var(--az-text)]">{item.quantity}</td>
                      <td className="py-1.5 px-2 text-right text-az-text-secondary">{fmtUSDC(item.unitCost)}</td>
                      <td className="py-1.5 pl-2 text-right text-[var(--az-text)] font-medium">{fmtUSDC(item.lineTotal)}</td>
                    </tr>
                  ))}
                </tbody>
                <tfoot>
                  <tr>
                    ...
                    <td className="py-2 pl-2 text-right text-[var(--az-text)] font-bold">{fmtUSDC(selectedPO.totalCost)}</td>
                  </tr>
                </tfoot>
              </table>
            </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-lg bg-[var(--az-accent)]/15 flex items-center justify-center flex-shrink-0">
                        <ClipboardCheck className="w-5 h-5 text-[var(--az-accent)]" />
                      </div>
                      <div>
                        <p className="text-sm font-medium text-[var(--az-text)]">{sc.countNumber}</p>
                  ...
                  {sc.status === 'OPEN' && sc.items && (
                    <div className="mt-2">
                      ...
                      <div className="h-1 bg-az-card rounded-full mt-1 overflow-hidden">
                        <div className="h-full bg-[var(--az-accent)] rounded-full transition-all" style={{ width: `${sc.items.length > 0 ? (sc.items.filter(i => i.countedQty !== null).length / sc.items.length * 100) : 0}%` }} />
                      </div>
                    </div>
                  )}
                </Card>
        ...
        <>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-sm font-medium text-[var(--az-text)]">{activeCount.countNumber}</h2>
                    ...
                    {activeCount.items?.map((item, idx) => {
                      ...
                      return (
                        <tr key={item.id} className="border-b border-az-border/30">
                          ...
                          <td className="py-2 px-3 text-[var(--az-text)] text-xs font-mono">{item.productId?.slice(0, 8)}…</td>
                          ...
                          <td className="py-2 px-3 text-right">
                            ...
                            ) : <span className="text-[var(--az-text)]">{item.countedQty ?? '—'}</span>}
                          </td>
                          <td className="py-2 px-3 text-right">
                            {discrepancy !== null ? (
                              <span className={`text-xs font-medium ${discrepancy === 0 ? 'text-az-text-muted' : discrepancy > 0 ? 'text-[var(--az-success)]' : 'text-[var(--az-danger)]'}`}>{discrepancy > 0 ? '+' : ''}{discrepancy}</span>
          ...
          {activeCount.status === 'RECONCILED' && (
            <div className="bg-[var(--az-success-subtle)] border border-emerald-500/30 rounded-lg p-3 flex items-center gap-2">
              <CheckCircle2 className="w-4 h-4 text-[var(--az-success)]" />
