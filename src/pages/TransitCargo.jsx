...
const STATUS_COLUMNS = [
  ...
  { key: 'LOADED', label: 'Loaded', color: 'var(--f-info)' },
  { key: 'IN_TRANSIT', label: 'In Transit', color: 'var(--f-tint-color)' },
  { key: 'DELIVERED', label: 'Delivered', color: 'var(--f-ok)' },
  { key: 'RETURNED', label: 'Returned', color: 'var(--f-warn)' },
  { key: 'LOST', label: 'Lost', color: 'var(--f-bad)' },
];
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-[var(--f-text)]">Cargo & Logistics</h1>
          <p className="text-sm text-[var(--f-text-3)] mt-0.5">Track parcels from dispatch to delivery</p>
        </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <Select value={filterStatus} onChange={e => setFilterStatus(e.target.value)}
          ...
          className="w-44" />
        <span className="text-sm text-[var(--f-text-3)]">{cargo.length} parcels</span>
      </div>
        ...
        {STATUS_COLUMNS.map(col => (
          <div key={col.key} className="space-y-2 min-w-[200px]">
            <div className="flex items-center justify-between px-1">
              <span className="text-sm font-bold text-[var(--f-text)]">{col.label}</span>
            ...
            {grouped[col.key]?.map(item => (
              <Card key={item.id} className="p-3 space-y-2">
                <div className="flex items-start justify-between">
                  <div>
                    <p className="text-sm font-bold text-[var(--f-text)]">{item.senderName}</p>
                    <p className="text-xs text-[var(--f-text-3)]">→ {item.receiverName}</p>
                  </div>
                  {item.fragile && <Tag variant="neutral">Fragile</Tag>}
                </div>
                <div className="space-y-1 text-xs">
                  <div className="flex justify-between"><span className="text-[var(--f-text-3)]">Weight</span><span className="text-[var(--f-text)] font-medium">{item.weightKg} kg</span></div>
                  <div className="flex justify-between"><span className="text-[var(--f-text-3)]">Price</span><span className="text-[var(--f-text)] font-medium">{fmtUSDC(item.priceUsdc)}</span></div>
                  {item.loadedAt && <div className="flex items-center gap-1 text-[var(--f-text-3)]"><Clock className="w-3 h-3" />Loaded {new Date(item.loadedAt).toLocaleDateString('en', { day: 'numeric', month: 'short' })}</div>}
                  {item.deliveredAt && <div className="flex items-center gap-1 text-[var(--f-ok)]"><CheckCircle2 className="w-3 h-3" />Delivered {new Date(item.deliveredAt).toLocaleDateString('en', { day: 'numeric', month: 'short' })}</div>}
                </div>
                {item.description && <p className="text-xs text-[var(--f-text-3)] truncate">{item.description}</p>}
                {item.proofOfDeliveryUrl && (
                  <a href={item.proofOfDeliveryUrl} target="_blank" rel="noopener" className="flex items-center gap-1 text-xs text-[var(--f-info)]:underline">
            ...
            {grouped[col.key]?.length === 0 && (
              <div className="text-center py-8 text-xs text-[var(--f-text-3)] border border-dashed border-[var(--f-line)] rounded-lg">
      ...
      {cargo.length === 0 && (
        <EmptyState icon={Package} title="No cargo parcels" description="Create a parcel to start tracking shipments" />
      )}
      ...
      <Modal open={!!proofOpen} onClose={() => { setProofOpen(null); setProofUrl(''); }} title="Attach Proof of Delivery">
        <div className="space-y-4">
          <p className="text-sm text-[var(--f-text-3)]">
            Attach a URL to the delivery photo or signature for parcel from <span className="text-[var(--f-text)] font-medium">{proofOpen?.senderName}</span> to <span className="text-[var(--f-text)] font-medium">{proofOpen?.receiverName}</span>.
          </p>
