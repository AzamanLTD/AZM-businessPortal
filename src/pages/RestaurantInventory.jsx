...
function StockBar({ current, minimum }) {
  ...
  return (
    <Progress
      value={pct}
      className="h-2 rounded-full bg-[var(--f-line)] overflow-hidden"
...
export default function RestaurantInventory() {
  ...
  if (!canView) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] text-center p-6 bg-[var(--f-bg)]">
        <AlertCircle className="w-12 h-12 text-[var(--f-bad)] mb-4" />
        <h2 className="text-lg font-bold text-[var(--f-text)]">Permission Denied</h2>
        <p className="text-sm text-[var(--f-text-3)] mt-1">You do not have permission to view or manage the restaurant inventory.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto px-4 md:px-6 py-6 text-[var(--f-text)]">
      {/* Page Header */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-[var(--f-text)] flex items-center gap-2">
            <Package className="w-6 h-6 text-[var(--f-tint-color)]" />
        ...
        {canManage && (
          <div className="flex gap-2 flex-wrap">
            ...
            <Button
              ...
              className="flex items-center gap-1.5 border-[var(--f-line)]:border-[var(--f-tint-color)] text-xs text-[var(--f-text-3)]"
            >
      ...
      {lowStockItems.length > 0 && (
        <Card className="border-[var(--f-bad)] bg-[rgba(239,68,68,0.03)] p-4 flex flex-col gap-3">
          <div className="flex items-center gap-2 text-[var(--f-bad)] font-semibold">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            {lowStockItems.map(item => {
              ...
              return (
                <div
                  key={item.id}
                  className="bg-[var(--f-ink-900)] border border-[var(--f-line)] p-3 rounded-xl flex flex-col justify-between"
                >
                  <div>
                    <div className="flex items-center justify-between gap-2 mb-1">
                      <span className="font-semibold text-sm truncate">{item.name}</span>
                      <Tag color={isOut ? 'var(--f-bad)' : 'var(--f-warn)'}>
                        {isOut ? '86 Needed' : 'Low'}
                      </Tag>
                    </div>
                    <p className="text-xs text-[var(--f-text-3)]">
                      Current: <strong className="text-[var(--f-text)]">{item.currentStock} {item.unit}</strong> (Min: {item.minimumStock})
                    </p>
                  </div>
                  {canManage && (
                    <div className="flex items-center gap-1.5 mt-3">
                      <button
                        ...
                        className="flex-1 bg-[var(--f-tint-color)]:bg-[#00c870] text-[var(--f-ink-900)] text-xs font-semibold py-1 rounded-lg transition-colors"
                      >
                        Restock
                      </button>
                      <button
                        onClick={() => quick86Mutation.mutate(item)}
                        className="px-2 py-1 bg-[rgba(239,68,68,0.1)]:bg-[var(--f-bad)] text-[var(--f-bad)]:text-[var(--f-text)] rounded-lg text-xs font-semibold transition-colors border border-[rgba(239,68,68,0.2)]"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="flex items-center gap-4 bg-[var(--f-surface)] p-4 border border-[var(--f-line)]">
          <div className="p-3 bg-[var(--f-line)] rounded-xl text-[var(--f-tint-color)]">
            <Package className="w-6 h-6" />
          </div>
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-[var(--f-text-3)]">Total Ingredients</p>
            <p className="text-xl font-bold f-mono mt-0.5">{itemsList.length}</p>
          </div>
        </Card>

        <Card className="flex items-center gap-4 bg-[var(--f-surface)] p-4 border border-[var(--f-line)]">
          <div className="p-3 bg-[rgba(245,158,11,0.1)] rounded-xl text-[var(--f-warn)]">
            <AlertTriangle className="w-6 h-6" />
          </div>
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-[var(--f-text-3)]">Low Stock Items</p>
            <p className="text-xl font-bold f-mono mt-0.5 text-[var(--f-warn)]">{lowStockItems.length}</p>
          </div>
        </Card>

        <Card className="flex items-center gap-4 bg-[var(--f-surface)] p-4 border border-[var(--f-line)]">
          <div className="p-3 bg-[rgba(239,68,68,0.1)] rounded-xl text-[var(--f-bad)]">
            <TrendingDown className="w-6 h-6" />
          </div>
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-[var(--f-text-3)]">Out of Stock</p>
            <p className="text-xl font-bold f-mono mt-0.5 text-[var(--f-bad)]">{outOfStockItems.length}</p>
          </div>
        </Card>

        <Card className="flex items-center gap-4 bg-[var(--f-surface)] p-4 border border-[var(--f-line)]">
          <div className="p-3 bg-[rgba(16,185,129,0.1)] rounded-xl text-[var(--f-ok)]">
            <DollarSign className="w-6 h-6" />
          </div>
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-[var(--f-text-3)]">Total Assets Value</p>
            <p className="text-xl font-bold f-mono mt-0.5 text-[var(--f-ok)]">GHS {totalValueGhs.toFixed(2)}</p>
          </div>
        </Card>
      </div>

      {/* Tabs Menu */}
      <div className="flex items-center justify-between border-b border-[var(--f-line)] pb-px">
            ...
            {CATEGORIES.map(cat => (
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-xs font-semibold whitespace-nowrap transition-all border ${
                  filterCat === cat
                    ? 'bg-[var(--f-tint-color)] text-[var(--f-ink-900)] border-[var(--f-tint-color)] font-bold'
            ...
            <div className="space-y-6">
              {Object.keys(groupedItems).map(category => (
                <div key={category} className="space-y-3">
                  <h3 className="text-xs font-bold uppercase tracking-wider text-[var(--f-text-3)] border-l-2 border-[var(--f-tint-color)] pl-2">
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    {groupedItems[category].map(item => {
                      ...
                      return (
                        <Card
                          key={item.id}
                          className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-[var(--f-surface)] p-4:border-[var(--f-tint-color)]/40 transition-colors"
                        >
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 flex-wrap">
                              <h4 className="font-semibold text-base text-[var(--f-text)] truncate">{item.name}</h4>
                              <Tag color={isOut ? 'var(--f-bad)' : isLow ? 'var(--f-warn)' : 'var(--f-ok)'}>
                                {isOut ? 'OUT' : isLow ? 'LOW' : 'IN STOCK'}
                              </Tag>
                              {item.supplier && (
                                <span className="text-[10px] bg-[var(--f-line)] text-[var(--f-text-3)] px-1.5 py-0.5 rounded flex items-center gap-1">
                                  <Truck className="w-3 h-3" />
                                  {item.supplier}
                                </span>
                              )}
                            </div>
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-4 gap-y-1 mt-2 text-xs text-[var(--f-text-3)]">
                              <div>
                                Cost per unit: <strong className="text-[var(--f-text)]">GHS {parseFloat(item.costPerUnit || 0).toFixed(2)} / {item.unit}</strong>
                              </div>
                              <div>
                                Supplier: <span className="text-[var(--f-text)] font-semibold">{item.supplier || 'N/A'}</span>
                              </div>
                              <div>
                                Total Value: <strong className="text-[var(--f-text)]">GHS {(cur * parseFloat(item.costPerUnit || 0)).toFixed(2)}</strong>
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="w-full md:w-48">
                            <div className="flex justify-between text-xs font-semibold mb-1">
                              ...
                              <span className="text-[var(--f-text-3)]">min: {min} {item.unit}</span>
                            </div>
                          ...
                          {canManage && (
                            <div className="flex items-center gap-2 self-end md:self-auto">
                              <Button
                                ...
                                className="flex items-center gap-1 border-[var(--f-line)] text-xs h-9"
                              >
                                <RefreshCw className="w-3.5 h-3.5" />
                                Restock
                              </Button>
      ...
      {activeTab === 'recipes' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
            <h3 className="text-base font-bold text-[var(--f-text)]">Restaurant Products Catalog</h3>
            {recipesLoading ? (
              <div className="space-y-3">
                <Skel className="h-16 w-full" />
                <Skel className="h-16 w-full" />
                    ...
                    <Card
                      ...
                      className={`cursor-pointer transition-all border p-4 ${
                        selectedProduct?.id === product.id
                          ? 'border-[var(--f-tint-color)] bg-[rgba(168,85,247,0.02)]'
                          : 'bg-[var(--f-surface)] border-[var(--f-line)]:border-[var(--f-tint-color)]/40'
                      }`}
                    >
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div>
                          <div className="flex items-center gap-2">
                            <h4 className="font-semibold text-base text-[var(--f-text)]">{product.name}</h4>
                            <span className="text-[10px] bg-[var(--f-line)] text-[var(--f-text-3)] px-1.5 py-0.5 rounded font-mono">
                              ID: {product.id}
                            </span>
                          </div>
                          <div className="flex gap-4 mt-2 text-xs">
                            <span className="text-[var(--f-text-3)]">
                              Sells: <strong className="text-[var(--f-text)]">GHS {sellingPrice.toFixed(2)}</strong>
                            </span>
                            <span className="text-[var(--f-text-3)]">
                              Cost: <strong className="text-[var(--f-text)]">GHS {totalCost.toFixed(2)}</strong>
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="text-right flex sm:flex-col items-center sm:items-end justify-between sm:justify-center gap-2">
                          <span className="text-xs text-[var(--f-text-3)] uppercase tracking-wider font-semibold">Margin</span>
                          <span className={`text-sm font-bold px-2 py-0.5 rounded-full ${
                            marginPct <= 20
                              ? 'bg-[rgba(239,68,68,0.1)] text-[var(--f-bad)]'
                              : marginPct <= 45
                                ? 'bg-[rgba(245,158,11,0.1)] text-[var(--f-warn)]'
                                : 'bg-[rgba(16,185,129,0.1)] text-[var(--f-ok)]'
                          }`}>
                            {marginPct.toFixed(0)}%
                          </span>
                        </div>
                      ...
                      <div className="mt-4 pt-3 border-t border-[var(--f-line)]">
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-xs font-semibold text-[var(--f-text-3)] flex items-center gap-1">
                            <Layers className="w-3.5 h-3.5" />
                            Ingredients Linked ({ingredients.length})
                          </span>
                        </div>
                        {ingredients.length === 0 ? (
                          <p className="text-xs text-[var(--f-text-3)] italic">No recipe elements configured yet.</p>
                        ) : (
                          <div className="flex flex-wrap gap-1.5">
                            {ingredients.map(ing => (
                              <Tag key={ing.inventoryItemId || ing.id} color="var(--f-tint-color)">
          ...
          <div className="space-y-4">
            <h3 className="text-base font-bold text-[var(--f-text)] flex items-center gap-1">
              <Scale className="w-5 h-5 text-[var(--f-tint-color)]" />
              Recipe Cost Sheet Details
            </h3>

            {selectedProduct ? (
              <Card className="bg-[var(--f-surface)] border-[var(--f-tint-color)] p-5 space-y-4 sticky top-4">
                <div className="flex items-start justify-between border-b border-[var(--f-line)] pb-3">
                  <div>
                    <h4 className="font-bold text-lg text-[var(--f-text)]">{selectedProduct.name}</h4>
                    <span className="text-xs text-[var(--f-text-3)] font-mono">Product ID: {selectedProduct.id}</span>
                  </div>
                  <button
                    onClick={() => setSelectedProduct(null)}
                    className="p-1.5:bg-[var(--f-line)] rounded-lg text-[var(--f-text-3)] transition-colors"
                  >
                    <X className="w-4 h-4" />
                  </button>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 bg-[var(--f-ink-900)] p-3 rounded-xl border border-[var(--f-line)] text-center">
                  <div>
                    <span className="text-xs text-[var(--f-text-3)]">Selling Price</span>
                    <p className="text-base font-bold text-[var(--f-text)] mt-0.5">
                      GHS {parseFloat(selectedProduct.priceUsdc || 0).toFixed(2)}
                    </p>
                  </div>
                  <div>
                    <span className="text-xs text-[var(--f-text-3)]">Ingredient Cost</span>
                    <p className="text-base font-bold text-[var(--f-tint-color)] mt-0.5">
                ...
                <div className="space-y-2">
                  <span className="text-xs font-bold text-[var(--f-text-3)] uppercase tracking-wider">
                    Linked Ingredients & Quantity
                  </span>

                  {(selectedProduct.recipeIngredients || selectedProduct.ingredients || []).length === 0 ? (
                    <div className="text-center py-6 border border-dashed border-[var(--f-line)] rounded-xl">
                      <p className="text-xs text-[var(--f-text-3)]">No ingredients linked yet.</p>
                    </div>
                  ) : (
                    <div className="divide-y divide-[var(--f-line)] bg-[var(--f-ink-900)] rounded-xl border border-[var(--f-line)] overflow-hidden">
                      {(selectedProduct.recipeIngredients || selectedProduct.ingredients || []).map(ing => {
                        ...
                        return (
                          <div key={ing.inventoryItemId || ing.id} className="p-3 flex items-center justify-between text-xs">
                            <div>
                              <p className="font-semibold text-[var(--f-text)]">{ing.inventoryItemName || ing.name}</p>
                              <p className="text-[10px] text-[var(--f-text-3)]">
                                {qty} {ing.unit} x GHS {itemCost.toFixed(2)}
                              </p>
                            </div>
                            <div className="flex items-center gap-3">
                              <span className="font-semibold text-[var(--f-text)]">GHS {lineCost.toFixed(2)}</span>
                              {canManage && (
                                <button
                                  ...
                                  className="text-[var(--f-bad)]:bg-[rgba(239,68,68,0.1)] p-1.5 rounded-lg transition-colors"
              ...
              <Card className="p-8 text-center border-dashed border-[var(--f-line)]">
                <p className="text-sm text-[var(--f-text-3)]">
      ...
      <Modal open={isCreateOpen} onClose={() => setIsCreateOpen(false)} title="Add Inventory Item">
        <form onSubmit={handleCreateSubmit} className="space-y-4">
          ...
          />
          <div className="flex justify-end gap-2 pt-2 border-t border-[var(--f-line)]">
      ...
      <Modal open={!!restockItem} onClose={() => setRestockItem(null)} title="Quick Restock">
        {restockItem && (
          <form onSubmit={handleRestockSubmit} className="space-y-4">
            <div className="p-3 bg-[var(--f-ink-900)] rounded-xl border border-[var(--f-line)]">
              <p className="text-xs text-[var(--f-text-3)] font-semibold uppercase tracking-wider">Item Details</p>
              <p className="text-base font-bold text-[var(--f-text)] mt-1">{restockItem.name}</p>
              <div className="grid grid-cols-2 gap-4 mt-2 text-xs">
                <div>
                  Current Stock: <strong className="text-[var(--f-text)]">{restockItem.currentStock} {restockItem.unit}</strong>
                </div>
                <div>
                  Unit Cost: <strong className="text-[var(--f-text)]">GHS {parseFloat(restockItem.costPerUnit || 0).toFixed(2)}</strong>
                </div>
              </div>
            </div>
            <Input
