            ...
            (openTabs || []).map((tab) => (
              <Card key={tab.id} onClick={() => { setSelectedTabId(tab.id); setSelectedTabLocationId(tab.locationId || ''); }} className={`cursor-pointer border p-4 ${selectedTabId === tab.id ? 'border-[var(--accent)]' : 'border-[var(--line)]'}`}>
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className="font-mono text-sm font-bold">USDC {money(tab.grandTotalUsdc || tab.subtotalUsdc)}</span>
                </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <Card className="border p-4">
                  ...
                  {(products || []).length === 0 ? <p className="py-8 text-center text-xs text-[var(--text-3)]">No products available for this branch.</p> : <div className="max-h-[400px] divide-y divide-[var(--line)] overflow-y-auto">{products.map((product) => <div key={product.id} className="flex items-center justify-between gap-3 py-2.5"><div><p className="text-sm font-semibold">{product.name}</p><p className="text-xs text-[var(--text-3)]">USDC {money(product.priceUsdc)}</p></div><Button variant="secondary" size="sm" onClick={() => { setActiveItemConfig(product); setItemQuantity(1); setItemNotes(''); }}><Plus className="h-4 w-4" /></Button></div>)}</div>}
                </Card>

                <Card className="border p-4">
                  <h4 className="mb-3 font-semibold">Running Bill</h4>
                  {(activeTabDetails.items || []).length === 0 ? <p className="py-8 text-center text-xs text-[var(--text-3)]">No items yet.</p> : <div className="max-h-[280px] divide-y divide-[var(--line)] overflow-y-auto">{activeTabDetails.items.map((item) => <div key={item.id} className="flex items-center justify-between gap-3 py-2.5 text-xs"><div><p className="font-semibold">{item.name}</p><p className="text-[10px] text-[var(--text-3)]">{item.quantity}x @ USDC {money(item.unitPriceUsdc)}</p></div><strong>USDC {money(item.lineTotalUsdc)}</strong></div>)}</div>}
                  <div className="mt-3 space-y-2 border-t border-[var(--line)] pt-3 text-sm"><div className="flex justify-between"><span className="text-[var(--text-3)]">Subtotal</span><strong>USDC {money(activeTabDetails.subtotalUsdc)}</strong></div>{activeTabDetails.status === 'BILLING' && <><div className="flex justify-between"><span className="text-[var(--text-3)]">Tax</span><strong>USDC {money(activeTabDetails.taxTotalUsdc)}</strong></div><div className="flex justify-between"><span className="text-[var(--text-3)]">Tip</span><strong>USDC {money(activeTabDetails.tipUsdc)}</strong></div></>}<div className="flex justify-between border-t border-[var(--line)] pt-2 font-bold"><span>Total</span><span>USDC {money(activeTabDetails.grandTotalUsdc || activeTabDetails.subtotalUsdc)}</span></div></div>
