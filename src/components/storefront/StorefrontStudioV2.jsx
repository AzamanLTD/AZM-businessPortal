...
const VIEWPORTS = [{ key:'phone', label:'Phone', Icon:Smartphone }, { key:'tablet', label:'Tablet', Icon:Tablet }, { key:'desktop', label:'Desktop', Icon:Monitor }];
...
function devicePreviewStyle(viewport) {
  ...
  return { width: device.displayWidthPx, height: Math.round(device.heightDp * STOREFRONT_STUDIO_TOKENS.PREVIEW_SCALE * scale) };
}

function StudioStage({ draft, business, document, selection, viewport, onSelect, onDropTile }) {
  const runtimeDraft = useMemo(() => {
    const next = studioDocumentToRuntimeDraft(draft, document, viewport);
    next.layoutJson.tiles = next.layoutJson.tiles.map((tile) => tile.widgetType === 'custom_html' ? { ...tile, props: { ...tile.props, html: sanitizeStorefrontPreviewHtml(tile.props?.html, 500) } } : tile);
    return next;
  }, [draft, document, viewport]);
  const selectedNode = selection[0] ? document.nodes[selection[0]] : null;
  const theme = useMemo(() => ({ name: draft?.themeName || 'Studio', tokenSet: document?.theme?.tokens || {} }), [draft?.themeName, document?.theme?.tokens]);
  ...
  return <div className="relative">
    <div className="pointer-events-none absolute left-1/2 top-0 z-10 -translate-x-1/2 -translate-y-1/2 rounded-full border px-2 py-1 text-[9px] font-semibold shadow-sm" style={{ borderColor:'var(--f-line)', background:'var(--f-surface)', color:'var(--f-text-3)' }}>{selectedNode ? `Editing ${LABELS[selectedNode.type] || selectedNode.type}` : 'Select a layer to edit'}</div>
    <div className="flex justify-center overflow-visible pt-2" onClick={(e) => e.stopPropagation()}>
      <div data-testid="studio-device-emulator" data-viewport={normalizedViewport} aria-label={`${device.widthDp} by ${device.heightDp} ${normalizedViewport} preview`} style={emulationSize}>
        <div style={{ width: phoneWidth, height: phoneHeight, transform: `scale(${scale})`, transformOrigin:'top left' }}>
          <StorefrontPhonePreview draft={runtimeDraft} theme={theme} widgets={[]} business={business} businessType={business?.businessType || business?.type || 'GENERAL'} selectedTileId={selection[0] || null} onSelectTile={onSelect} editorMode onDropTile={onDropTile} />
        </div>
      </div>
    </div>
  </div>;
...
export default function StorefrontStudioV2({ draft, saveDraft, business, onClose }) {
  ...
  const [tab, setTab] = useState('content'); const [filter, setFilter] = useState(''); const [viewport, setViewport] = useState('phone'); const [dragTarget, setDragTarget] = useState(null);
  const dragRef = useRef(null); const suppressClickRef = useRef(false);
  ...
  const select = (id, e) => { if (!id) return studio.setSelection([]); studio.setSelection((e?.metaKey || e?.ctrlKey) ? [...new Set([...studio.selection, id])] : [id]); };
  const add = (type, parentId = null, index = -1) => studio.addNode(type, parentId, index);
  const insertPaletteNode = (type, targetId, edge) => { if (!STUDIO_NODE_TYPES.includes(type) || !targetId) return; const parentId = getStudioParentId(studio.document, targetId); const siblingIds = parentId ? (studio.document.nodes[parentId]?.children || []) : (studio.document.pages?.[0]?.root || []); const targetIndex = siblingIds.indexOf(targetId); if (targetIndex < 0) return; add(type, parentId || null, targetIndex + (edge === 'after' ? 1 : 0)); };
  ...
  const finishPalettePointer = (event, cancelled = false) => {
    ...
    window.removeEventListener('pointermove', active.move); window.removeEventListener('pointerup', active.up); window.removeEventListener('pointercancel', active.cancel);
    dragRef.current = null;
    if (!cancelled && active.dragging && active.target) { insertPaletteNode(active.type, active.target.tileId, active.target.edge); suppressClickRef.current = true; window.setTimeout(() => { suppressClickRef.current = false; }, 0); }
    setDragTarget(null);
  };
  ...
  const startPalettePointer = (type, event) => {
    ...
    const move = (moveEvent) => {
      ...
      if (!target) { dragRef.current.target = null; setDragTarget(null); return; }
      ...
      dragRef.current.target = resolved; setDragTarget(resolved);
    };
  ...
  useEffect(() => { const normalized = normalizeStudioViewport(viewport); if (normalized !== viewport) setViewport(normalized); }, [viewport]);
  useEffect(() => { const handleKeyDown = (event) => { const target = event.target; const typing = target instanceof HTMLElement && (target.isContentEditable || ['INPUT','TEXTAREA','SELECT'].includes(target.tagName)); if (typing) return; const mod = event.metaKey || event.ctrlKey; if (mod && event.key.toLowerCase() === 'z') { event.preventDefault(); if (event.shiftKey) studio.redo(); else studio.undo(); return; } if (mod && event.key.toLowerCase() === 'y') { event.preventDefault(); studio.redo(); return; } if (mod && event.key.toLowerCase() === 'd' && node) { event.preventDefault(); studio.duplicate(); return; } if ((event.key === 'Delete' || event.key === 'Backspace') && node) { event.preventDefault(); studio.remove(); } }; window.addEventListener('keydown', handleKeyDown); return () => window.removeEventListener('keydown', handleKeyDown); }, [node, studio]);
